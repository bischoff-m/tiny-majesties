...
public class TilePainter : MonoBehaviour{
	...
	// CUSTOM
	public Quaternion fixedRotationApplied = Quaternion.identity;
	...
	public void Restore(){
		...
		if (tiles == null){
			...
			// CUSTOM
			tiles.transform.rotation = Quaternion.Inverse(fixedRotationApplied);
			var newTraining = tiles.AddComponent<Training>();
			newTraining.gridsize = gridsize;
			newTraining.width = width;
			newTraining.depth = height;
			if (outputObject.TryGetComponent(out OverlapWFC model))
			{
				model.training = newTraining;
			}
			// END CUSTOM
		}
		...
		if (this.ValidCoords((int)cursor.x, (int)cursor.y)){
			if (op == TileLayerEditor.TileOperation.Sampling){
				...
				if (op == TileLayerEditor.TileOperation.Drawing){
					...
					o.transform.localRotation *= fixedRotationApplied;
